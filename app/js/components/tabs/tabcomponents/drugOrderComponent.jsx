...
class DrugOrderComponent extends Component {
    ...
    componentWillMount() {
        this.props.fetchData('drug').then((drugs) => {
            ...
            Promise.all(conceptsGenerics).then(drugGenerics => {
                this.setState({drugs: drugs.results, generics: drugGenerics[0]});
            });
        }).catch((error) => {
            this.setState({error: true, message: error.message});
        });
        this.props.fetchData('concept?name=REASON%20ORDER%20STOPPED').then(conceptReason => {
            this.props.fetchData(`concept/${conceptReason.results[0].uuid}`).then(conceptDetails => {
                this.setState({reasons: conceptDetails.answers});
            });
        });
    ...
    showGenerics() {
        ...
        for (let uuid in this.state.generics) {
            genericsDisplay.push(
                <option value={uuid} key={shortid.generate()}>{this.state.generics[uuid]}</option>
            );
        }
    ...
    showOptions(selectData) {
        return selectData.map((eachOption) => <option value={eachOption.uuid} key={shortid.generate()}>{eachOption.display}</option>);
    }
